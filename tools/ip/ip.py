...
class Ip(object):
  ...
  def get_ip_address(type, pagenum, targeturl, path): # ip类型,页码,目标url,存放ip的路径

      url = self.proxy_ip_address[str(type)]+str(pagenum) # 配置url
  ...
  def __init__(self, logger, proxy_ip_address):
